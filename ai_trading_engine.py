...
def _is_valid_trading_env(trenv) -> bool:
    ...
    return all(hasattr(trenv, field) and getattr(trenv, field) not in (None, "") for field in required_fields)
...
def init_kis_api():
    ...
    try:
        ...
        if not ensure_kis_auth():
            logger.warning("KIS API 인증 초기화 지연: 인증 컨텍스트가 아직 유효하지 않습니다. 루프에서 재시도합니다.")
        database.init_db()
        return True
    except Exception as e:
